...
def test_wrong_length_frame_is_rejected():
    ...
    with pytest.raises(ValueError):
        decode_verdict(bytes(19))
    with pytest.raises(ValueError):
        decode_verdict(bytes(21))


def test_encode_clean_verdict_is_magic_then_zeros():
    assert encode_verdict() == bytes([0xA5]) + bytes(19)


def test_encode_packs_all_fields():
    frame = encode_verdict(bloom_hit=True, rate_anomaly=True, severity=3,
                           escalate=True, seq=42)
    assert frame == bytes([0xA5, 0b101, 3, 0b1, 42]) + bytes(15)


def test_encode_produces_20_bytes():
    assert len(encode_verdict(seq=255)) == 20
...
def test_describe_no_verdict():
    assert decode_verdict(bytes(20)).describe() == "no-verdict"
