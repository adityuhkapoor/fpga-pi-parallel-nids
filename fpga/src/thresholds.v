...
module thresholds (
    ...
    output wire [7:0]  rule_epoch     // v2 step 4: live tap for rule_lookup.v
);
    reg [15:0] port_r, host_r, rate_r, rule_epoch_r;
    assign port_thresh = port_r;
    assign host_thresh = host_r;
    assign rate_thresh = rate_r;
    assign rule_epoch  = rule_epoch_r[7:0];

    initial begin port_r = 16'd5; host_r = 16'd5; rate_r = 16'd8;
                  rule_epoch_r = 16'd0; r_val = 16'd0; end

    always @(posedge clk) begin
        if (rst) begin
            port_r <= 16'd5; host_r <= 16'd5; rate_r <= 16'd8; rule_epoch_r <= 16'd0;
        end else if (w_en) case (w_id)
            8'h00: port_r       <= w_val;
            8'h01: host_r       <= w_val;
            8'h02: rate_r       <= w_val;
            8'h03: rule_epoch_r <= w_val;
            default: ;
        endcase
        case (r_id)
            8'h00:   r_val <= port_r;
